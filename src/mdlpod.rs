...
pub fn extract_mdl_signature_data(
    ...
    let sig_r_s = match &issuer_auth[3] {
        serde_cbor::Value::Bytes(b) => {
            <&[u8; 64]>::try_from(&b[..]).map_err(|_| "signature length must be 64 bytes")?
        }
...
pub mod tests {
    ...
    use plonky2_ecdsa::{
        curve::{curve_types::AffinePoint, ecdsa::ECDSAPublicKey, p256::P256},
    ...
    fn test_mdl_pod_verify() -> Result<(), Box<DynError>> {
        // Load the MDL data
        let root: serde_cbor::Value =
            serde_cbor::from_reader(File::open("test_keys/mdl/decoded.bin")?)?;
    ...
    fn test_mdl_pod_with_mainpod() -> Result<(), Box<DynError>> {
        ...
        let msg_copy = main_pod_builder
            ...
            .unwrap();
        main_pod_builder.pub_op(op!(eq, (&main_mdl_pod, KEY_SIGNED_MSG), msg_copy))?;

        // Perpetuate the pk
        main_pod_builder.pub_op(op!(copy, main_mdl_pod.public_statements[2].clone()))?;
